...
for ifile in range(0,len(fcast_hr)):
    ...
    for ivar in range(0,len(var_dict)):
        ...
        plot_levels = np.linspace(0,500, nlev)                        
        ...
        cbar = plt.colorbar(contourf_plot, ax=ax, orientation='horizontal', ticks=np.arange(0,501,50))
        contourf_plot.set_clim(0, 500)
        cbar.set_label(f"{long_name} ({units})")
        ax.set_title(f"{long_name} at {fcast_hr[ifile]} hours")
        ax.set_xlabel('Longitude')
        ax.set_ylabel('Latitude')
        plt.subplots_adjust(left=0.05, right=0.9, top=0.15, bottom=0.1)
        plt.tight_layout()
        # Save
        print(fileOUT)
