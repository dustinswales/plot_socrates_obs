...
for ifile in range(0,len(fcast_hr)):
    ...
    # For each variable...
    for ivar in range(len(var_dict)):
        # Create output file.
        ...
        land_10m = cfeature.NaturalEarthFeature('physical', 'land', '10m', edgecolor='k', facecolor= 'none')
        ocean_10m = cfeature.NaturalEarthFeature('physical', 'ocean', '10m', edgecolor='k', facecolor= 'none')
        lakes_10m = cfeature.NaturalEarthFeature('physical', 'lakes', '10m', edgecolor='k', facecolor= 'none')
        c_10m = cfeature.NaturalEarthFeature('cultural', 'admin_0_countries', '10m', edgecolor='k', facecolor= 'none')

        ax.add_feature(land_10m,zorder=2)
        ax.add_feature(ocean_10m,zorder=3)
        ax.add_feature(lakes_10m,zorder=4)
        ax.add_feature(c_10m,zorder=5)

        # Make contour contourf
        contourf_plot = ax.contourf(lon, lat, varPlt, levels=plot_levels, cmap=cmap_seq, zorder=1)
